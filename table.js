...
export class Table extends EventTarget {
	...
	_save() {
		...
		this._stack[++this._stackIndex] = data;
	...
	_use(data) {
		...
		obj.rows?.forEach(row => {
			...
			row.cols?.forEach(cell => {
				const td = this._createCell(tr, undefined, cell.header);
				td.textContent = cell.data;
				if(cell.row_span) td.rowSpan = cell.row_span;
				if(cell.col_span) td.colSpan = cell.col_span;
			});
			tbody.appendChild(tr);
		});
	 ...
	 * @property {boolean} withSelected
	 * 
	 * @param {GetJSONOptions} options 
	 * @returns {string}
	 */
	getJSON(options) {
		const gt1 = v => { return v > 1 ? v : undefined; };
		const rows = [];
		Array.from(this.table.rows).forEach(row => {
			...
			Array.from(row.cells).forEach(cell => {
				const c = {
					...
					row_span: gt1(cell.rowSpan),
					col_span: gt1(cell.colSpan),
				};
				...
				if(options?.withSelected) {
					const selected = this._isSelected(cell);
					if(selected) {
						c.selected = true;
					}
				}
	 ...
	 * @param {boolean} header 是否是表头元素。
	 * @returns {HTMLTableCellElement}
	 */
	_createCell(row, pos, header) {
		// row.insertCell 无法创建 th，所以不用。
		const cell = document.createElement(header ? 'th' : 'td');
		if(this._fill_with_nbsp) {
			cell.innerHTML = '&nbsp;';
		}

		// pos== -1 || undefined 时在末尾插入。
		const ref = row.children[pos] || null;
